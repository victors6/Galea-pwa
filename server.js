...
app.get("/", (req, res) => {
  res.status(200).json({ ok: true, service: "galea-backend" });
});
...
// Analizar factura con imagen base64
app.post("/scan-factura", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res.status(422).json({ error: "No se pudo parsear respuesta de IA", raw });
    }
    res.json(parsed);
  } catch (err) {
    res.status(500).json({ error: "Error interno", detail: err.message });
...
// Arrancar — usar PORT de Railway obligatoriamente
const PORT = parseInt(process.env.PORT, 10) || 3000;
app.listen(PORT, "0.0.0.0", () => {
  console.log(`Galea backend corriendo en puerto ${PORT}`);
});
